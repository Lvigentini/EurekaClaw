---
name: eurekaclaw
description: EurekaClaw project context — architecture, pipeline, CLI commands, data models, and development patterns. Use when working on any EurekaClaw code.
user-invocable: true
---

# EurekaClaw Development Context

## What This Is
EurekaClaw is a multi-agent AI research system that goes from a question to a publishable result. It crawls literature, generates hypotheses, proves theorems, runs experiments, and writes LaTeX papers.
...
### Central Hub: KnowledgeBus (`eurekaclaw/knowledge_bus/bus.py`)
...
### Version Store (`eurekaclaw/versioning/`)
Every `persist_incremental()` auto-commits a version. Users can:
- `eurekaclaw history <session>` — view timeline
- `eurekaclaw diff <session> v1 v2` — compare versions
- `eurekaclaw checkout <session> v3` — rollback (preserves HEAD as new version)

### Paper Model (`eurekaclaw/types/artifacts.py`)
```python
...
### IdeationPool (`eurekaclaw/orchestrator/ideation_pool.py`)
...
```bash
eurekaclaw prove "conjecture" --domain "ML theory"
eurekaclaw explore "domain topic"
eurekaclaw from-papers 2401.12345 --domain "ML theory"
eurekaclaw from-bib refs.bib --pdfs ./papers/ --domain "ML theory"
eurekaclaw from-draft paper.tex "Strengthen the theory" --domain "ML theory"
eurekaclaw from-zotero COLLECTION_ID --domain "ML theory"
```

### Mid-Session Injection (pause first with Ctrl+C)
```bash
eurekaclaw inject paper SESSION_ID 2401.12345
eurekaclaw inject idea SESSION_ID "What if we use spectral methods?"
eurekaclaw inject draft SESSION_ID paper.tex "Consider these results"
```

### Version Management
```bash
eurekaclaw history SESSION_ID
eurekaclaw diff SESSION_ID 1 3
eurekaclaw checkout SESSION_ID 2
...
eurekaclaw from-zotero COLLECTION_ID --domain "ML theory"
eurekaclaw push-to-zotero SESSION_ID --collection "Results"
```
...
def persist_incremental(self, ...):
    from eurekaclaw.versioning.store import VersionStore  # lazy
```
